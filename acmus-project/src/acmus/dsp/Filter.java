...
public class Filter {
  ...
  static {
    ...
    if (f instanceof DecimalFormat) {
      ...
      System.err.println("Warning: class Filter: not instance of DecimalFormat");
  ...
  public String bToString() {
    StringBuffer sb = new StringBuffer();
    for (int i = 0; i< b.length; i++) sb.append(FORMAT.format(b[i]) + " ");
    return sb.toString();
  }
  public String aToString() {
    StringBuffer sb = new StringBuffer();
    for (int i = 0; i< a.length; i++) sb.append(FORMAT.format(a[i]) + " ");
    return sb.toString();
  }
  ...
  public static double[] filtfilt(double[] b, double[] a, double[] x) {
    ...
    //
    // % References:
    // % [1] Sanjit K. Mitra, Digital Signal Processing, 2nd ed., McGraw-Hill,
    // 2001
    // % [2] Fredrik Gustafsson, Determining the initial states in
    // forward-backward
    // % filtering, IEEE Transactions on Signal Processing, pp. 988--992, April
    // 1996,
    // % Volume 44, Issue 4
    //
    ...
    // end
    if (len <= nfact) {
      throw new Error("Data must have length more than 3 times filter order. "
          + len + " " + nfact);
    }
    //
  ...
  public static final double[] filter(double b[], double a[], double x[], double y[]) {
...
//    // [z,p,k] = buttap(n);
    ...
    double k = buttap(n, pRe, pIm);
//  
//    // % Transform to state-space
//    // [a,b,c,d] = zp2ss(z,p,k);
//    //
//    // % step 4: Transform to lowpass, bandpass, highpass, or bandstop of
//    // desired Wn
//    // if btype == 1 % Lowpass
