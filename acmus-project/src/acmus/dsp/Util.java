...
public final class Util {
  ...
  public static void wavWrite(double t[], int channels, int bitsPerSample,
    ...
    if (bitsPerSample == 32) {
      for (int i = 0; i < t.length; i++) {
        ...
        // if (Math.abs(t[i]) == 1) {
        // System.out.println("sample: " + t[i] + " " + sample + " " + res[i] +
        // " " + res[2*i]);
        // }
      }
    ...
    try {
      ...
      AudioFormat format = new AudioFormat(44100, bitsPerSample, channels,
          true, false);
      AudioInputStream ais = new AudioInputStream(baos, format, t.length);
      if (AudioSystem.write(ais, AudioFileFormat.Type.WAVE, new File(filename)) == -1) {
  ...
  public static void wavWrite(int t[], int channels, int bitsPerSample, String filename) {
    byte res[] = new byte[t.length * (bitsPerSample / 8)];
    ...
    if (bitsPerSample == 32) {
      ...
      /* Second byte is MSB (high order) */
      res[i * 2 + 1] = (byte) (sample >> 8);
      // System.out.println( t[i] + " " + sample + " " + res[i] + " " res[2*i]);
      }
    }
    try {
      ByteArrayInputStream baos = new ByteArrayInputStream(res);
      AudioFormat format = new AudioFormat(44100, bitsPerSample, channels, true, false);
      AudioInputStream ais = new AudioInputStream(baos, format, t.length/channels);
      if (AudioSystem.write(ais, AudioFileFormat.Type.WAVE, new File(filename)) == -1) {
        throw new IOException("Problems writing to file");
      }
    } catch (Exception e) {
  ...
  public final static void wavWrite(byte data[], double rate, int bitsPerSample, int channels, boolean bigEndian, String filename) {
	    try {
	        ByteArrayInputStream baos = new ByteArrayInputStream(data);
	        AudioFormat format = new AudioFormat((float)rate, bitsPerSample, channels, true, bigEndian);
	        AudioInputStream ais = new AudioInputStream(baos, format, data.length);
	        if (AudioSystem.write(ais, AudioFileFormat.Type.WAVE, new File(filename)) == -1) {
	          throw new IOException("Problems writing to file");
	        }
	      } catch (Exception e) {
  ...
  public final static double[][] wavReadSplitDouble(String filename) {
    ...
    try {
      AudioInputStream ais = AudioSystem
          .getAudioInputStream(new File(filename));
      int data[][] = splitAudioStream(ais.getFormat().getChannels(), AudioPlayer
          .readData(ais));
      res = new double[data.length][data[0].length];
      for (int i = 0; i < res.length; i++) {
        AudioPlayer.normalizeInPlace(res[i], data[i], ais.getFormat().getSampleSizeInBits());
      }
      ais.close();
    } catch (Exception e) {
      e.printStackTrace();
...
//    int i = 0;
  ...
  public static final void convolve(String input1, String input2,
    ...
    try {
      AudioInputStream ais1 = AudioSystem.getAudioInputStream(new File(input1));
      AudioInputStream ais2 = AudioSystem.getAudioInputStream(new File(input2));
      ...
      for (int i = 0; i < aa.length; i++) {
        for (int j = 0; j < bb.length; j++) {
          ...
          IProgressMonitor subMonitor = Util.subMonitorFor(monitor, 10);
...
//  public static double[] average(double[]... arrays) {
  ...
  public static void wavAverage(String outFile, int bitsPerSample, List<String> files) {
    String[] f = new String[files.size()];
    f = files.toArray(f);
    wavAverage(outFile, bitsPerSample, f);
  }

  public static void wavAverage(String outFile, int bitsPerSample, String... files) {
