...
//        System.out.println("Plot disposed.");
  ...
  public void paintControl(PaintEvent e) {
    ...
    gc.fillRectangle(_xFrame, _yFrame, _coords.width() - 2 * _xFrame, _coords
        .height()
        - 2 * _yFrame);

    // drawing area border
    gc.setForeground(AcmusGraphics.BLACK);
    gc.drawRectangle(_xFrame, _yFrame, _coords.width() - 2 * _xFrame, _coords
        .height()
        - 2 * _yFrame);
    // 
    ...
    gc.fillRectangle(_coords.width() - _xFrame + 1, _yFrame, _xFrame, _coords
        .height()
        - _yFrame);
    gc.fillRectangle(_xFrame, _coords.height() - _yFrame + 1, _coords.width()
        - _xFrame, _yFrame);
    ...
    while (xm <= _coords.xMax()) {
      ...
      gc.drawString(str, x - (gc.stringExtent(str).x / 2), y,
          true);
      xm += _xStep;
    }
    Point strLen = gc.stringExtent(_xLabel);
    gc.drawString(_xLabel, _coords.width() / 2 - strLen.x / 2, _coords.height() - strLen.y);
    ...
    while (ym <= _coords.yMax()) {
      ...
      gc.drawString(str, x - strLen.x - 8, y
          - strLen.y / 2, true);
      ym += _yStep;
    }
...
class Line extends Dots {
  ...
  public void draw(GC gc) {
    ...
    for (int i = 1; i < _x.length; i++) {
      gc.drawLine(_coords.x(_x[i - 1]), _coords.y(_y[i - 1]), _coords.x(_x[i]),
          _coords.y(_y[i]));
    }
  }
}
