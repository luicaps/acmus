...
public class FilterBank {
  ...
  static {
    ...
    try {
      u = AcmusPlugin.getDefault().getBundle().getEntry("data/filters/1d8.txt");
      ...
      br = new BufferedReader(new InputStreamReader(u.openStream()));
      // br = new BufferedReader(new FileReader(__p + "/sweep-butter.txt"));
      readSweepButter(br);

      u = AcmusPlugin.getDefault().getBundle()
          .getEntry("data/filters/comp.txt");
      br = new BufferedReader(new InputStreamReader(u.openStream()));
    ...
    try {
      ...
      while (line != null) {
        ...
        if (st.countTokens() != b.length) {
          System.err.println("warning: butter: " + n + " " + st.countTokens());
        }
  ...
  private static final void readSweepButter(BufferedReader br) {
    try {
      ...
      while (line != null) {
        ...
        st = new StringTokenizer(line);
        if (st.countTokens() != b.length) {
          System.err.println("warning: sbutter: " + n + " " + st.countTokens());
        }
        for (int i = 0; i < b.length; i++) {
          b[i] = Double.parseDouble(st.nextToken());
        }
    ...
    try {
      ...
      while (line != null) {
        ...
        if (type.equals("a")) {
          ...
          System.err.println("warning: comp filter: unknown: " + type);
        }

        line = br.readLine();
        st = new StringTokenizer(line);
        if (st.countTokens() != b.length) {
          System.err.println("warning: comp: " + type + " " + st.countTokens());
        }
        for (int i = 0; i < b.length; i++) {
          b[i] = Double.parseDouble(st.nextToken());
        }
