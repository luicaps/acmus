...
public class FFT1d extends TempVars {
	...
	public void fft(double inputRe[], double inputIm[]) {
		...
		if (inputRe.length != N || inputIm.length != N) {
			System.out
					.println("Error: the length of real part & imaginary part "
			...
			for (int groupNo = 0; groupNo < remainRadix; groupNo++) {
				...
				if ((sofarRadix > 1) && (dataNo > 0)) {
					...
					do {
						...
						temRe[blockIndex] = twiddleRe[blockIndex]
								* outputRe[address] - twiddleIm[blockIndex]
								* outputIm[address];
						temIm[blockIndex] = twiddleRe[blockIndex]
								* outputIm[address] + twiddleIm[blockIndex]
								* outputRe[address];
						blockIndex++;
