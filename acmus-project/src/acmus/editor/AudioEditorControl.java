...
public class AudioEditorControl extends Composite {
  ...
  Combo cFilters;
  String filterList[] = { "62.5", "125", "250", "500", "1000", "2000", "4000", "8000" };
  ...
  public AudioEditorControl(Composite parent, int style, String dir, String file, boolean barOnTop,
      boolean selection, boolean saveBar) {
    super(parent, style);
  ...
  private void createDisplays() {
    ...
    _wf = _ap.createWaveformDisplay(_waveformComposite,
        SWT.NONE);
    gridData = new GridData(GridData.FILL_BOTH);
    ...
    _wf.setSelectionEndDisplayListener(_ap.createSelectionEndDisplayListener());
    ...
    _graphMode.addSelectionListener(new SelectionAdapter() {
      public void widgetSelected(SelectionEvent event) {
        ...
        case 0:
          ...
          _graphArea.layout();
          // _waveform.amplitude(); <-- already called by default listener
  ...
  private void createSpectrum() {
    ...
    Text t = _ap.createSpectrumFrequency(_spectrumComposite, SWT.BORDER | SWT.READ_ONLY);
    gridData = new GridData();
    gridData.widthHint = 40;
    t.setLayoutData(gridData);
    t = _ap.createSpectrumDb(_spectrumComposite, SWT.BORDER | SWT.READ_ONLY);
    gridData = new GridData();
  ...
  private void createBar(boolean selection) {
    ...
    if (selection) {
      ...
      _tSelStart.setToolTipText("Selection start position in milliseconds");
      gridData = new GridData();
      gridData.widthHint = 50;
      ...
      _tSelEnd.addKeyListener(new KeyListener() {
        ...
        public void keyReleased(KeyEvent e) {
          System.out.println(e.keyCode);
          if (e.keyCode == SWT.CR) {
            _wf.setSelectionEndInMillis(Integer.parseInt(_tSelEnd.getText()));
          }
        }
      });
    }
  ...
  public void createSaveBar() {
    ...
    bFilter.addSelectionListener(new SelectionAdapter() {
      public void widgetSelected(SelectionEvent event) {
        Filter f = FilterBank.get1d8(Double.parseDouble(filterList[cFilters.getSelectionIndex()]), _wf.getSampleRate());
        _wf.filter(f);
        _wf.save(_dir + System.getProperty("file.separator") + "temp.wav");
        _ap.open(_dir + System.getProperty("file.separator") + "temp.wav");
      }
    });
    ...
    cut.addSelectionListener(new SelectionAdapter() {
      public void widgetSelected(SelectionEvent event) {
        if (selectionValid()) {
          _wf.cut(getSelectionStartInSamples(), getSelectionEndInSamples());
          _wf.save(_dir + System.getProperty("file.separator") + "temp.wav");
          _ap.open(_dir + System.getProperty("file.separator") + "temp.wav");
          eraseSelection();
        }
