...
public class ImpulseResponsePage extends Composite {
	...
	private void createPage() {
		...
		this._aeIrLf = new AudioEditorControl(c, SWT.NONE, true, false,
		false);
		gridData = new GridData(GridData.FILL_BOTH);
		...
		_bParam.setToolTipText(
				"Calculate acoustical parameteres using selected IR");
	...
	public void calculateParameters(MeasurementEditor parent) {
		try {
			...
			if (getAeIr().selectionValid()) {
				ir2 = Util.subArray(ir2, getAeIr().getSelectionStartInSamples(),
						getAeIr().getSelectionEndInSamples());
			}
			...
			if ("Chu".equals(_cParamMethod.getItem(_cParamMethod
				...
				p.chuParam(this._directSound, this._firstReflection, outTable,
					...
					.getSelectionIndex()))) {
				Parameters.lundebyParam(ir2, ir2Lf, 44100, this._directSound,
						this._firstReflection, outTable, parent.getSchroederFolder()
								.getLocation().toOSString());
			} else if ("Hirata".equals(_cParamMethod.getItem(_cParamMethod
					.getSelectionIndex()))) {
				Parameters.hirataParam(ir2, ir2Lf, 44100, this._directSound,
						this._firstReflection, outTable, parent.getSchroederFolder()
								.getLocation().toOSString());
			} else {
				...
				Parameters.chuParamOld(ir2, ir2Lf, 44100, this._directSound,
						this._firstReflection, outTable, parent.getSchroederFolder()
			...
			if (!parent.getParamsFile().exists()) {
				parent.getParamsFile().create(new ByteArrayInputStream(buf), true, null);
			} else {
				parent.getParamsFile().setContents(new ByteArrayInputStream(buf), true,
						true, null);
			}
