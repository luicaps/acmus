...
public class SchroederDiffuser extends Composite {
  ...
  public SchroederDiffuser(Composite parent, int style) {
    ...
    _result = new Text(sf, SWT.MULTI | SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL);
  ...
  public boolean validate() {
    ...
    if (Double.isNaN(gap)) {
      _result.setText("Error: \"Distance between wells\" must be a number.");
      return false;
    }
  ...
  public static void main(String args[]) {
    ...
    for (int i = 0; i < 2 * N; i++) {
      int sn = (i * i % N);
      System.out.println(i + ". " + sn + " " + (double) (sn * c) / (2 * N * f0)
          + " " + s[i % s.length]);
    }
  }
}
...
class DiffuserPanel extends Canvas implements PaintListener {
  ...
  public void setData(double[] depths, double W, double T, double periods, boolean drawWall) {
    _drawWall = drawWall;
    _periods = periods;
    double mmPerPixel = T;
    ...
    for (int i = 0, x = 1; i < depths.length; i++, x += pixelsPerW + 1) {
      gc.fillRectangle(x, 1 + imgHeight
          - (int) Math.round(depths[i] / maxD * imgHeight), pixelsPerW,
          1 + imgHeight);
    }
  ...
  public void paintControl(PaintEvent e) {
    ...
    if (wScale < hScale) {
      ...
      int start = (int)Math.round((width / 2) - (imgWidth/2)- ( ( Math.ceil(_periods)-1) / 2 * imgWidth)) + 1;
      int end = (int)((width / 2) + (imgWidth/2) + ( (Math.ceil(_periods)-1) / 2 * imgWidth));
      for (int x = start; x < end; x += imgWidth) {
        gc.drawImage(img, 0, 0, img.getBounds().width, img.getBounds().height,
            x, 1 + (height - imgHeight) / 2, imgWidth, imgHeight);
      }
