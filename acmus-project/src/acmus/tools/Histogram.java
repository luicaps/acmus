...
public class Histogram extends Canvas implements PaintListener {
  ...
  public void paintControl(PaintEvent e) {
    ...
    gc.drawLine(width - _xroff + _arrowFoot, height - (_yboff + _arrowHeadW),
        width - _xroff + _arrowHeadH + _arrowFoot, height - _yboff);
    gc.drawLine(width - _xroff + _arrowFoot, height - (_yboff - _arrowHeadW),
        width - _xroff + _arrowHeadH + _arrowFoot, height - _yboff);
    ...
    for (int i = 0; i < _labels.length; i++) {
      int x = _xloff + (i + 1) * (width - _xloff - _xroff) / (_labels.length);
      if (!"".equals(_labels[i]))
    ...
    for (Line l : _data) {
      ...
      for (int i = 1; i < l.dataY.length; i++) {
        ...
        if (l.drawLabels) {
          ...
          gc.drawString(l.topLabels[i], x - (labelSize.x / 2), y - labelSize.y,
              true);
        }
        path.lineTo(x, y);
      }
...
class PostScriptWriter {
  ...
  public PostScriptWriter(PrintStream out, String title, int width, int height,
      double fontSize) {
    ps = out;
    this._width = width;
    this._height = height;
