...
public class ResonanceFrequency implements IWorkbenchWindowActionDelegate {
  ...
  SelectionAdapter computeListener = new SelectionAdapter() {
    ...
    private void drawGraph(Histogram hist, Vector val, Label label, String str1, String str2) {
      if (val.isEmpty() == true) {
        label.setText("All " + str1 + " frequencies are greater than 300Hz");
        ...
        for (int i = 0; i < val.size(); i++) {
          dataX[i] = ((Double) val.elementAt(i)).doubleValue();
          dataY[i] = 1;
          int number = 0;
          for (int j = 0; j < val.size(); j++) {
            double aux1 = ((Double) val.elementAt(i))
                .doubleValue();
            double aux2 = ((Double) val.elementAt(j))
                .doubleValue();
            if (aux1 == aux2)
              number++;
          }
    ...
    private void drawGraph2(Histogram hist, Vector val, Label label, String str1, String str2) {
      if (val.isEmpty() == true) {
        label.setText("All " + str1 + " frequencies are greater than 300Hz");
        hist.setData(emptyDataX, emptyDataY, emptyTopLabels);
      } else {
    ...
    public void widgetSelected(SelectionEvent event) {
      try {
        ...
        if (cal.returnInputValidity() == false)
          ...
          drawGraph(hAxial,cal.axial_frequency,lAxial,"axial","Axial");
