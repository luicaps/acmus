...
public class CompareWaveforms extends Composite {
  ...
  public void open(String filename) {
    try {
      ...
      if (_waveform.numberOfArrays() < 1) {
        _waveform.setData(audioData, audioStream.getFormat().getChannels(),
            audioStream.getFormat().getSampleRate(), audioStream.getFormat().getSampleSizeInBits(), getColor(0));

      } else {
        _waveform.addData(audioData, audioStream.getFormat().getChannels(),
            audioStream.getFormat().getSampleRate(), getColor(_waveform
                .numberOfArrays()));
      }
      ...
      Object o = new TableLine(name,getColor(_waveform
          .numberOfArrays() - 1));
      _tViewer.add(o);
  ...
  private void createTableViewer(Composite parent) {
    _table = new Table(parent, SWT.SINGLE | SWT.CHECK | SWT.BORDER | SWT.V_SCROLL
        | SWT.H_SCROLL);
    ...
    _tViewer.addCheckStateListener(new ICheckStateListener() {
      ...
      public void checkStateChanged(CheckStateChangedEvent event) {
        for (int i = 0; i< _waveform.numberOfArrays(); i++ ){
          _waveform.drawArray(i, _tViewer.getChecked(_tViewer.getElementAt(i)));
        }
      }
