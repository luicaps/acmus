...
public class ReverberationTime extends Composite {
  ...
  public ReverberationTime(Shell shell, Composite parent, int style,
    ...
    _scrolledSurfaces = new ScrolledComposite(g, SWT.V_SCROLL | SWT.H_SCROLL);
    ...
    Surface s = new Surface(_surfacesPanel, _s.size() + 1, _surfCoefClasses,
        _surfCoefNames, _surfCoefs);
    _s.add(s);
    ...
    // _output = new Text(c, SWT.BORDER | SWT.MULTI | SWT.WRAP | SWT.READ_ONLY
    // | SWT.V_SCROLL);
  ...
  public static double airAbsorption(double freq, double temp, double humidity,
      double pressure, Text output) {
    double Pso = 101325;
    double Tt = 273.16;
    ...
    Frn = pressure / Pso * Math.sqrt(T20 / T)
        * (9 + 350 * h * Math.exp(-6.142 * (Math.pow(T20 / T, 1.0 / 3) - 1)));
  ...
  private void export() {
    ...
    if (filename != null) {
      try {
        histogram.exportPs(new PrintStream(new FileOutputStream(filename)));
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
  ...
  private void compute() {
    ...
    if (_method.getSelectionIndex() == 0) {
      ...
      for (int i = 0; i < _time.length; i++) {
        ...
        double alphaNp = airAbsorption(freqs[i], temp, humidity, pressure,
            _output);
      ...
      for (int i = 0; i < _time.length; i++) {
        ...
        _time[i] = -0.161 * volume / ((totalArea * Math.log(1 - alpham)) + air);
        _output.append(freqs[i] + "Hz:\n");
        _output.append("   A_air = " + air + "\n");
        _output.append("    RT60 = " + _time[i] + "\n");
        _output.append("\n");
    ...
    Object o = new TableLine("" + (_table.getItemCount() + 1), getColor(_table
        .getItemCount()));
    _tViewer.add(o);
...
class Surface {
  ...
  public Surface(Composite parent, int id, String[] classes,
    ...
    _materialClass.addSelectionListener(new SelectionListener() {
      ...
      public void widgetSelected(SelectionEvent e) {
        _material.setItems(_materials[_materialClass.getSelectionIndex()]);
        _material.select(0);
        update();
      }
    });
  ...
  public void update() {
    String matClass = _materialClass
        .getItem(_materialClass.getSelectionIndex());
...
class Obj extends Surface {
  ...
  public Obj(Composite parent, int id, String[] classes, String[][] materials,
      Map<String, Map<String, double[]>> coefs) {
