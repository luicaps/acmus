...
public class CalculateFrequency {
  ...
  public void calculateAxialFrequency() {
    ...
    if (input_valid == true) {
      for (p = 1; p <= (int) 300.0 * 2.0 * length / v; p++) {
        double frequency = v / 2.0 * Math.sqrt((p * p) / (length * length));
        axial_frequency.addElement(new Double(frequency));
      }
      for (q = 1; q <= (int) 300.0 * 2.0 * width / v; q++) {
        double frequency = v / 2.0 * Math.sqrt((q * q) / (width * width));
        axial_frequency.addElement(new Double(frequency));
      }
      for (r = 1; r <= (int) 300.0 * 2.0 * height / v; r++) {
        double frequency = v / 2.0 * Math.sqrt((r * r) / (height * height));
        axial_frequency.addElement(new Double(frequency));
      }
    }
  ...
  public void calculateTangentialFrequency() {
    ...
    if (input_valid == true) {
      if (1.0 / (width * width) <= 600.0 / v
        ...
        double x = width
            * Math.sqrt(600.0 * 600.0 / (v * v) - 1.0 / (height * height));
        for (q = 1; q <= (int) x; q++) {
          double y = height
              * Math.sqrt(600.0 * 600.0 / (v * v) - 1.0 / (width * width));
          for (r = 1; r <= (int) y; r++) {
            double frequency = v
                / 2.0
                * Math.sqrt((r * r) / (height * height) + (q * q)
                    / (width * width));
            if (frequency <= 300.0) {
              tangential_frequency.addElement(new Double(frequency));
            }
          }
        }
      }
      if (1.0 / (length * length) <= 600.0 / v
        ...
        for (p = 1; p <= (int) x; p++) {
          double y = height
              * Math.sqrt(600.0 * 600.0 / (v * v) - 1.0 / (length * length));
          for (r = 1; r <= (int) y; r++) {
            double frequency = v
                / 2.0
                * Math.sqrt((r * r) / (height * height) + (p * p)
                    / (length * length));
            if (frequency <= 300.0) {
              tangential_frequency.addElement(new Double(frequency));
            }
  ...
  public void calculateObliqueFrequency() {
    ...
    if (input_valid == true) {
      if (v
          * Math.sqrt(1 / (height * height) + 1 / (length * length) + 1
              / (width * width)) / 2.0 <= 300.0) {
        double x = length
            * Math.sqrt(600.0 * 600.0 / (v * v) - 1.0 / (height * height) - 1.0
                / (width * width));
        for (p = 1; p <= (int) x; p++) {
          double y = width
              * Math.sqrt(600.0 * 600.0 / (v * v) - 1.0 / (height * height)
          ...
          for (q = 1; q <= (int) y; q++) {
            ...
            for (r = 1; r <= (int) z; r++) {
              double frequency = v
                  / 2.0
                  * Math.sqrt((q * q) / (width * width) + (p * p)
                      / (length * length) + (r * r) / (height * height));
              if (frequency <= 300.0) {
                oblique_frequency.addElement(new Double(frequency));
              }
            }
          }
        }
