...
public class SpectrumDisplay extends Composite {
  ...
  class Spectrum extends Composite {
    ...
    public void setData(double[][] points) {
      ...
      for (int i = 0; i < points.length; i++) {
        for (int j = 0; j < points[i].length; j++) {
          ...
          // gc.drawLine(i, points[i].length - j - 1, i, points[i].length - j);
    ...
    class Spectrum3D extends Canvas implements PaintListener, MouseListener,
        MouseMoveListener {
      ...
      public void paintControl(PaintEvent e) {
        ...
        _zMark = _spectrumPoints[_xMark][_spectrumPoints[0].length - _yMark - 1];
        ...
        // -1 - (int)(_zMark * (AcmusGraphics.SPECTRUM_COLOR_SCALE.length-1))]);
      ...
      public final void draw(GC gc, int x, int y, int width, int height) {
        ...
        gc.drawImage(_spec3DImg, _left, _top, _viewWidth, _viewHeight, x, y,
            width, height);
      }
      ...
      public void mouseMove(MouseEvent e) {
        ...
        if (_positionDisplay != null) {
          _positionDisplay.setText(""
              + Math.round(_xMark / 2 * _stepSize / _sampleRate * 1000));
        }
        if (_freqDisplay != null) {
          _freqDisplay
              .setText(""
                  + Math
                      .round((1 - (double) _yMark / (_bucketHeight - 1)) * 20000));
        }
        if (_dbDisplay != null) {
      ...
      public void setZoomWindow(int x, int y, int width, int height) {
        ...
        _viewHeight = (int) Math.round(_viewHeight * (double) height / h);
...
//      }
      ...
      public final void draw(GC gc, int x, int y, int width, int height) {
        Display d = AcmusPlugin.getDefault().getWorkbench().getDisplay();
        double yScale = height / (double) _spectrumPoints[_xMark].length;
        double xScale = width;
        ...
        for (int j = 1; j < _spectrumPoints[_xMark].length; j++) {
          int x2 = (int) Math.round(_spectrumPoints[_xMark][j] * xScale);
        ...
        gc.drawImage(AcmusGraphics.IMG_SPECTRUM_COLOR_SCALE, 0, 0,
            AcmusGraphics.SPECTRUM_COLOR_SCALE.length, 1, 0, 0, width, height);
        ...
        gc.setAlpha(120);
        gc.setForeground(AcmusGraphics.LIGHT_BLUE);
        gc.drawLine(0, (int) Math.round(_yMark * yScale), width, (int) Math
            .round(_yMark * yScale));
        // gc.setForeground(AcmusGraphics.SPECTRUM_COLOR_SCALE[AcmusGraphics.SPECTRUM_COLOR_SCALE.length
        // -1 - (int)(_zMark * (AcmusGraphics.SPECTRUM_COLOR_SCALE.length-1))]);
        gc
        ...
        gc.drawLine(0, (int) Math.round(_top * yScale) - 1, width, (int) Math
            .round(_top * yScale) - 1);
        gc.drawLine(0, (int) Math.round((_top + _viewHeight) * yScale) - 1,
            width, (int) Math.round((_top + _viewHeight) * yScale) - 1);
  ...
  public void setData(int data[], int channels, float sampleRate,
    ...
    for (int ch = 0; ch < _channels; ch++) {
      ...
      for (int i = 0; i < _spectrumPoints.length; i++) {
        ...
        computeSpectrum(d, _stepSize, _bucketHeight, 20000, _stepSize, 44100.0,
            _spectrumPoints[i]);
        start += _stepSize / 2;
