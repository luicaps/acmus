...
public class WaveformDisplay extends Composite {
  ...
  public void power() {
    ...
    setYMax((int) Math.ceil(_sampleT.transf((1 << (_bitsPerSample - 1)) - 1)));
  ...
  public void setZoom(double xZoom, double yZoom) {
    _hbar.setMaximum(10 + (int) Math.round(_drawingAreaWidth * (xZoom - 1)));
    _hbar.setSelection((int) ((double) _xvStart * xZoom / _xZoom));
    _xvStart = (int) ((double) _xvStart * xZoom / _xZoom);

    _vbar.setMaximum(10 + (int) Math.round(_drawingAreaHeight * (yZoom - 1)));
  ...
  public final void updateYBar() {
    ...
    double yPos = (double) _vbar.getSelection() / _vbar.getMaximum() * newYMax;
  ...
  public void yFit2() {
    ...
    if (Double.isInfinite(max - min)) {
      ...
      setYPos((int) ((_drawingAreaHeight * _yZoom)
          * ((_sampleT.unitMaxValue() - max) / _sampleT.unitValueRange()) - 0.05 * _drawingAreaHeight));
    }
  ...
  public int positionInMillis(int x) {
    int samples = (int) Math
        .round((_xvStart + x - _yBarWidth) * _samplesPerDot);
  ...
  public void filter(Filter f) {
    ...
    for (SampleArray s: _sampleArrays ) {
      ...
      for (int i = 0; i < streams.length; i++) {
        double x[] = Util.normalize(streams[i], max);
        x = Filter.filtfilt(f.b, f.a, x);
        Util.revertNormalize(x, streams[i], max);
      }
...
//    }
//    Util.wavWrite(Util.joinAudioStream(streams), streams.length, _bitsPerSample, filename);
    
    // save only the first samplearray
    Util.wavWrite(_sampleArrays.get(0).data, _sampleArrays.get(0).channels, _bitsPerSample, filename);
  }
  /* ======================================================================= */
  ...
  class Waveform extends Canvas implements ControlListener, PaintListener,
    ...
    public void paintControl(PaintEvent e) {
      int _width = this.getBounds().width;
      int _height = this.getBounds().height;

      _samplesPerDot = ((double) _numSamples / (_width - _yBarWidth)) / _xZoom;

      // _timeBar.update(_width, _xvStart *_samplesPerDot/_sampleRate, _width *
      ...
      if (AcmusGraphics.DOUBLE_BUFFER) {
        buffer = new Image(
            AcmusPlugin.getDefault().getWorkbench().getDisplay(), _width,
            _height);
        gc = new GC(buffer);
      ...
      for (SampleArray sa : _sampleArrays) {
        ...
        if (_samplesPerDot < 1) {
          ...
          if (((int) Math.round(_xvStart * _samplesPerDot)) * channels + _ch < data.length) {

            int step = (int) Math.round(1 / _samplesPerDot);
//            int xShift = (int) Math
//                .round(((1 - (((double) _xvStart / step) - Math.floor(_xvStart
//                    / step))))
//                    * step);
            ...
            {
              x1 = 0;
              y1 = (int) Math.round((_yMax - _sampleT.transf(data[((int) Math
                  .round(_xvStart * _samplesPerDot))
                  * channels + _ch]))
                  * scale)
                  - yShift;
              for (int i = (int) Math.round(_xvStart * _samplesPerDot) + 1, k = step; i < data.length
                  / channels
                  && k < _width + step; i++, k += step) {

                x2 = k;
                y2 = (int) Math.round((_yMax - _sampleT.transf(data[i
                    * channels + _ch]))
                    * scale)
                    - yShift;
                gc.drawLine(x1 + _yBarWidth, y1, x2 + _yBarWidth, y2);
                x1 = x2;
                y1 = y2;
              }
            }
            ...
            if (_dotRadius * 4 < step) {
              for (int i = (int) Math.round(_xvStart * _samplesPerDot), k = 0; i < data.length
                ...
                gc.fillOval(x2 - _dotRadius + _yBarWidth, y2 - _dotRadius,
                    _dotRadius * 2, _dotRadius * 2);
                gc.setForeground(AcmusGraphics.BLUE2);
                gc.drawOval(x2 - _dotRadius + _yBarWidth, y2 - _dotRadius,
                    _dotRadius * 2, _dotRadius * 2);
          ...
          if (_xvStart * _samplesPerDot * channels + _ch < data.length) {
            ...
            // initial position
            {
              ...
              for (int j = (int) i; j < i + _samplesPerDot; j++) {
                if (_sampleT.transf(data[j * channels + _ch]) > _sampleT
                    .transf(data[iMax]))
                  iMax = j * channels + _ch;
                else if (_sampleT.transf(data[j * channels + _ch]) < _sampleT
                    .transf(data[iMin]))
                  iMin = j * channels + _ch;
              }
              int left = (iMax < iMin) ? iMax : iMin;
              x1 = 0;
              y1 = (int) Math.round((_yMax - _sampleT.transf(data[left]))
                  * scale)
                  - yShift;
            }
            ...
            for (int i = (int) (_xvStart * _samplesPerDot), k = 0; i <= data.length
                ...
                && k < _width; i = (int) Math.round((_xvStart * _samplesPerDot)
                + (_samplesPerDot * ++k))) {
              ...
              x2 = k;
              y2 = (int) Math.round((_yMax - _sampleT.transf(data[left]))
                  * scale)
                  - yShift;
              ...
              // + " " +y2
              // + " " + _height + " " + _data[left]);
              drawLine(gc, color, color, x1 + _yBarWidth, y1, x2 + _yBarWidth,
                  y2);
              x1 = x2;
              y1 = y2;
              y2 = (int) Math.round((_yMax - _sampleT.transf(data[right]))
                  * scale)
                  - yShift;
              drawLine(gc, color, color, x1 + _yBarWidth, y1, x2 + _yBarWidth,
                  y2);
      ...
      if (_winSelWidth > 0) {
        ...
        gc.drawRectangle(_winSelX, _winSelY, _winSelWidth, _winSelHeight);
      }
      if (_xMark >= 0) {
        gc.setForeground(AcmusGraphics.GREEN);
        gc.drawLine(_xMark + _yBarWidth, 0, _xMark + _yBarWidth, _height);
      }
      if (_selSampleStart >= 0) {
        int xS = _yBarWidth - _xvStart
            + (int) Math.floor((double) _selSampleStart / _samplesPerDot);
        int xE = _yBarWidth - _xvStart
            + (int) Math.floor((double) _selSampleEnd / _samplesPerDot);
        gc.setForeground(AcmusGraphics.YELLOW);
      ...
      Transform oldTr = new Transform(AcmusPlugin.getDefault().getWorkbench()
          .getDisplay());
      gc.getTransform(oldTr);
      Transform tr = new Transform(AcmusPlugin.getDefault().getWorkbench()
          .getDisplay());
      tr.translate(2, _height / 2 + fm.getAverageCharWidth() * yLabel.length()
          / 2);
      tr.rotate(-90);
      gc.setTransform(tr);
      ...
      double stepLen = (double) drawingAreaHeight()
      ...
      int initialGapLen = (int) Math.round(initialGapVal
          * (drawingAreaHeight() * _yZoom) / _sampleT.unitValueRange());
      ...
      for (int i = 0; i <= steps; i++) {
        ...
        gc.drawString(l, _yBarWidth - 8
            - (fm.getAverageCharWidth() * l.length()), y - fm.getHeight() / 2,
            true);
      }

      if (AcmusGraphics.DOUBLE_BUFFER) {
        e.gc.drawImage(buffer, 0, 0, _width, _height, 0, 0, _width, _height);
        gc.dispose();
        buffer.dispose();
      }
    ...
    public void controlResized(ControlEvent e) {
      updateBars();
      _drawingAreaHeight = this.getBounds().height;
      _drawingAreaWidth = this.getBounds().width - _yBarWidth;
    }
  ...
  class TimeBar extends Canvas implements PaintListener {
    ...
    public void paintControl(PaintEvent e) {
      ...
      _samplesPerDot = ((double) _numSamples / (width - _yBarWidth)) / _xZoom;
      ...
      double stepLen = (double) (width - _yBarWidth) / (dur / stepDur);
      int initialGapLen = (int) Math.round((initialGapDur / stepDur) * stepLen);
      ...
      if (steps > 0) {
        ...
        for (int i = -1; i < steps; i++) {
          gc.setForeground(AcmusGraphics.WHITE);
          int x = initialGapLen + _yBarWidth + (int) Math.round(i * stepLen);
          ...
          gc.drawString(l, x - strLen.x / 2, height - 6 - strLen.y, true);
        }
      }
...
class HorizontalBar extends Canvas implements PaintListener {
  ...
  public final void update(int xStart, int xEnd, double valStart, double valEnd) {
  ...
  public void paintControl(PaintEvent e) {
    ...
    while (x < width) {
      ...
      if (val > _valMin) {
        ...
        gc.drawString(l, x - strSize.x / 2, height - 6 - strSize.y, true);
      }
      x += xStepLen;
    }
  }
